...
<div id="kmn-wrap">
  <style>
    ...
    .kmn-location-thumb-frame {
      position: relative; width: 260px; height: 180px; border-radius: 10px; overflow: hidden;
      border: 2px solid rgba(255,255,255,.35); box-shadow: 0 4px 14px rgba(0,0,0,.35);
      background: #1e293b;
    }
    .kmn-location-thumb-img { position: absolute; inset: 0; width: 100%; height: 100%; object-fit: cover; display: block; }
    .kmn-location-thumb-img.kmn-thumb-img-failed { display: none; }
    .kmn-location-thumb-overlay { position: absolute; inset: 0; width: 100%; height: 100%; pointer-events: none; }
    ...
    (function () {
      ...
      function renderSlides() {
        ...
        slidesEl.innerHTML = farms.map(function (f, i) {
          ...
          // only added when a location was found. The photo is a plain
          // <img>; a separate transparent <svg> is layered on top purely
          // to draw the polygon outline, using preserveAspectRatio=
          // "xMidYMid slice" so its 900x600 coordinate space crops in
          // lockstep with the img's own object-fit:cover crop underneath
          // -- keeping the outline aligned to the photo. If the photo
          // itself fails to load (network/CORS/service issue), only the
          // <img> is hidden (via a CSS class, NOT removed from the DOM)
          // -- the tinted box, the polygon outline and the caption stay
          // visible either way, so a photo failure never blanks out the
          // whole card.
          let locationThumbHtml = '';
          if (f.map_image) {
            const polyPoints = polygonPointsAttr(f.map_polygon, f.map_bbox);
            const polygonOverlaySvg = polyPoints
              ? '<svg class="kmn-location-thumb-overlay" viewBox="0 0 ' + mapImageWidth + ' ' + mapImageHeight + '" '
            ...
            locationThumbHtml = '<div class="kmn-location-thumb-wrap">'
              ...
              + '<img class="kmn-location-thumb-img" src="' + f.map_image + '" alt="Farm location" '
              + 'onerror="this.classList.add(\'kmn-thumb-img-failed\');" />'
              + polygonOverlaySvg
              + '</div>'
